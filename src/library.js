...
LibraryManager.library = {
  ...
  _tzset_js__deps: ['$tzsetImpl'],
  _tzset_js__sig: 'vppp',
  _tzset_js: function(timezone, daylight, tzname) {
    // TODO: Use (malleable) environment variables instead of system settings.
    if (__tzset_js.called) return;
    __tzset_js.called = true;
    tzsetImpl(timezone, daylight, tzname);
  },

  $tzsetImpl__internal: true,
  $tzsetImpl__proxy: 'sync',
  $tzsetImpl__deps: [
#if MINIMAL_RUNTIME
    '$allocateUTF8'
#endif
  ],
  $tzsetImpl: function(timezone, daylight, tzname) {
...
// N.b. Wasm workers do not provide this kind of clock synchronization.
  ...
  emscripten_get_callstack: function(flags, str, maxbytes) {
    // Use explict calls to pointerToDouble / doubleToPointer rather then
    // using the __sig magic here.  This is because the __sig wrapper uses
    // arrow function notation which causes the inner call to traverseStack
    // to fail.
    {{{ pointerToDouble('str') }}};
  ...
  $readAsmConstArgs: function(sigPtr, buf) {
    ...
    while (ch = HEAPU8[sigPtr++]) {
      ...
      assert(['p', 'd', 'f', 'i', 'j'].includes(chr), 'Invalid character ' + ch + '("' + chr + '") in readAsmConstArgs! Use only "d", "f", "p", "j" or "i", and do not specify "v" for void return argument.');
#if !WASM_BIGINT
      assert(chr !== 'j', "i64 arguments to ASM_JS function are not available without WASM_BIGINT");
#endif
#endif
#if MEMORY64
      if (ch == 112/*p*/) {
